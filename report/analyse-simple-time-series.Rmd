...
\rhead{Simulated Sockeye}
\pagestyle{fancy}
\thispagestyle{empty}

\begin{center}
{\LARGE Simulated Sockeye Salmon Spawners\\
  ~\\
...
Going to simulate Sockeye Salmon spawner abundances from Carrie's
{\tt salmonTraj()} function, plot in various ways, and use EDM to make
predictions. \\
...
x_t = N_{t+1} - N_t
\ee
for $t=1, ..., T-1$; writing it this way means that {\tt x[1]} is $x_1$ (so it
...
<<rotatable, echo=FALSE, results='hide', eval=FALSE>>=
# Plot lagged data in 3d space and can rotate it - useful to see structure
#  Note: different axes to plot3dmovie
plot3d(x[-c(1, 2)], x[-c(1, n)], x[-c(n-1, n)],
       xlab = "x(t)",
       ylab = "x(t-1)",
       zlab = "x(t-2)")
@

If want to plot a rotatable
3d plot then run the R code in chunk {\tt rotatable}.
