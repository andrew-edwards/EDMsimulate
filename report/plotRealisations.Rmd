...
p1 <- discrete_sens_pe %>% ggplot(aes(x = model_label, y = value, fill = forMethod)) +
  geom_violin(position = dodge, width=1.3,  aes(alpha = 0.5))+
  geom_boxplot(width = .1, outlier.colour = NA, position = dodge) +
	ylim(-100,100) + ylab("Percent error")
p2 <- discrete_sens_ae %>% ggplot(aes(x = model_label, y = value, fill = forMethod)) +
  geom_violin(position = dodge, width=1.2,  aes(alpha = 0.5))+
  geom_boxplot(width = .1, outlier.colour = NA, position = dodge) +
	ylim(0,1)+ylab("Absolute error")
...
p2 <- sigma_nu_sens_ae %>% ggplot(aes(x = model_label, y = value, fill = forMethod)) +
  geom_violin(position = dodge, width=1,  aes(alpha = 0.5))+
  geom_boxplot(width = .1, outlier.colour = NA, position = dodge) +
	ylim(0,1)+ylab("Absolute error")
...
# Extract forecasts and forecast errors
results_alpha_decline_sigma_nu_0.7 <- forecast_errors(out, label = "sigma_nu_0.7")
...
alpha_decline_sigma_nu_sens_pe <- results_alpha_decline$errors %>% dplyr::filter(error == "perErr") %>% 
	...
	add_row(results_alpha_decline_sigma_nu_0.7$errors %>% dplyr::filter(error == "perErr")) %>%
	dplyr::mutate(model_label=factor(model_label, 
																	 levels=c("alpha_decline_sigma_nu_0.1", "sigma_nu_0.3", "sigma_nu_0.5",
																	 				 "sigma_nu_0.7", "alpha_decline")))
...
alpha_decline_sigma_nu_sens_ae <- results_alpha_decline$errors %>% dplyr::filter(error == "absErr") %>% 
	...
	add_row(results_alpha_decline_sigma_nu_0.7$errors %>% dplyr::filter(error == "absErr")) %>%
	dplyr::mutate(model_label=factor(model_label, 
																	 levels=c("alpha_decline_sigma_nu_0.1", "sigma_nu_0.3", "sigma_nu_0.5",
																	 				 "sigma_nu_0.7", "alpha_decline")))
