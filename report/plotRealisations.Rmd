...
# Suggestion to put code below into a function:
...
```
The above example assumes returns are the variable being predictd. Another option is recruitment (by brood year), which is the more direct predictor of Larkin model. The simulation is repeated with recruitment.
...
# Get results for either R_prime (recruitment) or R_ (returns)
if(names(run2$res_realisations)[2]=="R_prime_T_sim"){
	res_realisations_plot2 <- run2$res_realisations %>% 
		dplyr::rename( R_sim = R_prime_T_sim) %>% 
		dplyr::rename( R_edm = R_prime_T_edm_fit) %>% 
		dplyr::rename( R_lar = R_prime_T_lar_fit) %>% 
		dplyr::rename( R_ric = R_prime_T_ric_fit)
}
if(names(run2$res_realisations)[2]=="R_T_sim"){
	res_realisations_plot2 <- run2$res_realisations %>% 
		dplyr::rename( R_sim = R_T_sim) %>% 
		dplyr::rename( R_edm = R_T_edm_fit) %>% 
		dplyr::rename( R_lar = R_T_lar_fit) %>% 
...
run.out2 <- res_realisations_plot2  %>% dplyr::mutate(absErr_edm = 
														 	(R_edm - R_sim), 
														 absErr_lar = 
														 	(R_lar - R_sim),
														 absErr_ric = 
...
absErr2 <- tidyr::pivot_longer(run.out2, c(absErr_edm, absErr_lar, absErr_ric), 
																names_to="forMethod", 
																names_prefix = "absErr_", 
																values_to="absErr")
absErr2 <- absErr2 %>% select(m,forMethod, absErr)

forecast_data2 <- tidyr::pivot_longer(run.out2, c(R_edm, 
																						R_lar,
																						R_ric), 
															names_to="forMethod", 
															names_prefix = "R_", 
...
forecast_data2 <- forecast_data2 %>% 
	select(m,forMethod, R_fit, R_sim)

p3 <- forecast_data2 %>% ggplot(aes(x=R_sim, 
																	 y=R_fit, 
																	 colour=forMethod)) + 
	geom_point() + 
	geom_smooth(method='lm', formula= y~x) + 
...
# Get results for either R_prime (recruitment) or R_ (returns)
if(names(run3$res_realisations)[2]=="R_prime_T_sim"){
	res_realisations_plot3 <- run3$res_realisations %>% 
		dplyr::rename( R_sim = R_prime_T_sim) %>% 
		dplyr::rename( R_edm = R_prime_T_edm_fit) %>% 
		dplyr::rename( R_lar = R_prime_T_lar_fit) %>% 
		dplyr::rename( R_ric = R_prime_T_ric_fit)
}
if(names(run3$res_realisations)[2]=="R_T_sim"){
	res_realisations_plot3 <- run3$res_realisations %>% 
		dplyr::rename( R_sim = R_T_sim) %>% 
		dplyr::rename( R_edm = R_T_edm_fit) %>% 
		dplyr::rename( R_lar = R_T_lar_fit) %>% 
...
run.out3 <- res_realisations_plot3 %>% dplyr::mutate(absErr_edm = 
														 	(R_edm - R_sim), 
														 absErr_lar = 
														 	(R_lar - R_sim),
														 absErr_ric = 
...
absErr3 <- tidyr::pivot_longer(run.out3, c(absErr_edm, absErr_lar, absErr_ric), 
																names_to="forMethod", 
																names_prefix = "absErr_", 
																values_to="absErr")
absErr3 <- absErr3 %>% select(m,forMethod, absErr)

forecast_data3 <- tidyr::pivot_longer(run.out3, c(R_edm, 
																						R_lar,
																						R_ric), 
															names_to="forMethod", 
															names_prefix = "R_", 
...
p4 <- forecast_data3 %>% ggplot(aes(x=R_sim, 
																	 y=R_fit, 
																	 colour=forMethod)) + 
	geom_point() + 
	geom_smooth(method='lm', formula= y~x) + 
