...
if(names(out$res_realisations)[2]=="R_prime_T_sim"){
	res_realisations_plot <- out$res_realisations %>% 
...
if(names(out$res_realisations)[2]=="R_T_sim"){
	res_realisations_plot <- out$res_realisations %>% 
...
out.df <- res_realisations_plot %>% dplyr::mutate(absErr_edm = 
														 	(R_edm - R_sim), 
														 absErr_lar = 
														 	(R_lar - R_sim),
														 absErr_ric = 
...
absErr <- tidyr::pivot_longer(out.df, c(absErr_edm, absErr_lar, absErr_ric), 
																names_to="forMethod", 
...
forecast_data <- tidyr::pivot_longer(out.df, c(R_edm, 
																						R_lar,
																						R_ric), 
															names_to="forMethod", 
															names_prefix = "R_", 
...
p3 <- forecast_data %>% ggplot(aes(x=R_sim, 
																	 y=R_fit, 
																	 colour=forMethod)) + 
	geom_point() + 
	geom_smooth(method='lm', formula= y~x) + 
	...
	geom_abline(slope=1, intercept = 0, col="dark grey")
p3
p4 <- absErr %>% ggplot(aes(x=absErr, colour=forMethod)) + 
	geom_density() +
	xlab("Absolute error in forecast") +
	ylab("Density")
p4
...
p4 <- forecast_data %>% ggplot(aes(x=R_sim, 
																	 y=R_fit, 
																	 colour=forMethod)) + 
	geom_point() + 
	geom_smooth(method='lm', formula= y~x) + 
	...
	geom_abline(slope=1, intercept = 0, col="dark grey")
p4
p5 <- absErr %>% ggplot(aes(x=absErr, colour=forMethod)) + 
	geom_density() +
	xlab("Absolute error in forecast") +
	ylab("Density")
p5
