...
## To run multiple simulations using the same parameter values but differing in stochasticity
...
Does this work? Thinking of using the term `realisations` for multiple runs that
have the same parameters but only differ in the resulting process noise
(i.e. just different due to stochasticity). Then say `simulations` for when changing different
assumptions (parameter values), and keep `scenarios` for the specific assumptions on
